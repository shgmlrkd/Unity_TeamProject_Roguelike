...
public class BossHealState : BossBase
{
    ...
    private IEnumerator HealCoroutine()
    {
        ...
        // 페이드 아웃
        bossContext.RequestFade(0.0f, FADE_DURATION);
        yield return fadeWait;

        // 힐 및 외형 변경
        bossContext.RequestHeal();
        bossContext.RequestSpriteLibraryChange(bossContext.data.AngrySpriteLibrary);

        // 변신 딜레이
        yield return phaseChangeWait;

        // 페이드 인
        bossContext.RequestFade(1.0f, FADE_DURATION);
        yield return fadeWait;
