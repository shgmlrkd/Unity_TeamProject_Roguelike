...
public class RoomBuilder
{
    ...
    public RoomBuilder(RoomGenerator gen, DirectionalRoomDatabaseSO db)
    {
        generator = gen;
        database = db;
    }
    ...
    {
        RoomType nextType = RoomRuleChecker.Instance.IsTimeForBossRoom() ? RoomType.Boss : RoomRuleChecker.Instance.DetermineNextRoomType();
        ...
        if (nextType == RoomType.Boss) RoomRuleChecker.Instance.CanGenerateMoreRooms = false;

        if (nextType == RoomType.Normal) RoomRuleChecker.Instance.GeneratedNormalRoomCount++;
        RoomRuleChecker.Instance.CurrentDoorUsedCount++;
