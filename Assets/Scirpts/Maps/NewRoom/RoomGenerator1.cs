...
public class RoomGenerator1 : MonoBehaviour
{
    ...
    {
        if (!RoomRuleChecker1.Instance.CanGenerateMoreRooms && roomInfo.type != RoomType.Boss) return null;
        if (roomInfo.type == RoomType.Boss && !RoomRuleChecker1.Instance.CanGenerateMoreRooms) return null;

        GameObject roomObj = Instantiate(roomInfo.roomPrefab, worldPos, Quaternion.identity);

        if (roomInfo.type == RoomType.Boss)
            RoomRuleChecker1.Instance.CanGenerateMoreRooms = false;
