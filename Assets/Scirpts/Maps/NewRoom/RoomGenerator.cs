...
public class RoomGenerator : MonoBehaviour
{
    ...
    {
        if (!RoomRuleChecker.Instance.CanGenerateMoreRooms && roomInfo.type != RoomType.Boss) return null;
        if (roomInfo.type == RoomType.Boss && !RoomRuleChecker.Instance.CanGenerateMoreRooms) return null;

        GameObject roomObj = Instantiate(roomInfo.roomPrefab, worldPos, Quaternion.identity);

        if (roomInfo.type == RoomType.Boss)
            RoomRuleChecker.Instance.CanGenerateMoreRooms = false;
